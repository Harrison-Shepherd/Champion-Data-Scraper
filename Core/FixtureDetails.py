...
class Fixture:
    """
    This class is responsible for fetching fixture data for a given league and fixture ID.
    """


    def __init__(self, league_id, fixture_id, regulation_periods, info_logger, error_logger):
        """
        Initialize the Fixture object with the league ID, fixture ID, regulation periods, and logger objects
        
        Parameters:
        league_id (int): The ID of the league.
        fixture_id (int): The ID of the fixture.
        regulation_periods (int): The number of regulation periods.
        info_logger (logging.Logger): The logger object for info messages.
        error_logger (logging.Logger): The logger object for error messages.
        """
    ...
    def fetch_data(self):
        ...
        if 'fixture' in data and 'match' in data['fixture']:
            ...
            self.info_logger.info(f"Total matches retrieved: {len(matches)}")
            self.info_logger.debug(f"Matches data: {matches}")
            
            # Proceed only if matches are available
            if matches:
                matches_df = pd.DataFrame(matches)
                self.info_logger.info(f"Total matches before filtering: {len(matches_df)}")
                self.info_logger.debug(f"Matches DataFrame columns: {matches_df.columns.tolist()}")
                self.info_logger.debug(f"Unique match statuses: {matches_df['matchStatus'].unique()}")
                
                # Filter out incomplete and scheduled matches
                matches_df = matches_df[~matches_df['matchStatus'].isin(['incomplete', 'scheduled'])]
                self.info_logger.info(f"Total matches after filtering: {len(matches_df)}")
                self.info_logger.debug(f"Filtered match statuses: {matches_df['matchStatus'].unique()}")

                # **Check if matches_df is empty after filtering**
                if matches_df.empty:
                    self.error_logger.error(f"No matches available after filtering for league {self.league_id}.")
                    self.data = pd.DataFrame()
                    return
            ...
            # Log error if no match data is found
            else:
                self.error_logger.error(f"No match data found for league {self.league_id}.")
                self.data = pd.DataFrame()
                return
            
        # Log error if the data is not in the expected format
        else:
